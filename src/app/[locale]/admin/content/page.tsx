...
import { Plus, Map, Building2, X, Newspaper } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useTranslations } from 'next-intl';

export default function ContentCMSPage() {
  const t = useTranslations('Admin.content');
  const [activeTab, setActiveTab] = useState<'destinations' | 'partners' | 'newsroom'>('destinations');
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex items-center gap-6 border-b border-zinc-200 overflow-x-auto">
        <button 
          ...
          className={`flex items-center gap-2 pb-3 text-sm font-medium transition-colors border-b-2 -mb-[1px] whitespace-nowrap ${activeTab === 'destinations' ? 'border-black text-zinc-900' : 'border-transparent text-zinc-500 hover:text-zinc-700'}`}
        >
          <Map className="w-4 h-4" /> {t('destinations')}
        </button>
        <button 
          onClick={() => setActiveTab('partners')}
          className={`flex items-center gap-2 pb-3 text-sm font-medium transition-colors border-b-2 -mb-[1px] whitespace-nowrap ${activeTab === 'partners' ? 'border-black text-zinc-900' : 'border-transparent text-zinc-500 hover:text-zinc-700'}`}
        >
          <Building2 className="w-4 h-4" /> {t('partners')}
        </button>
        <button 
          onClick={() => setActiveTab('newsroom')}
          className={`flex items-center gap-2 pb-3 text-sm font-medium transition-colors border-b-2 -mb-[1px] whitespace-nowrap ${activeTab === 'newsroom' ? 'border-black text-zinc-900' : 'border-transparent text-zinc-500 hover:text-zinc-700'}`}
        >
          <Newspaper className="w-4 h-4" /> Newsroom & Press
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeTab === 'destinations' && <DestinationsCMS t={t} />}
          {activeTab === 'partners' && <PartnersCMS t={t} />}
          {activeTab === 'newsroom' && <NewsroomCMS />}
        </motion.div>
      </AnimatePresence>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-zinc-200 shadow-sm p-12 text-center flex flex-col items-center justify-center min-h-[300px]">
        ...
        <h3 className="text-sm font-semibold text-zinc-900 mb-1">No articles published</h3>
        <p className="text-zinc-500 font-medium text-xs max-w-sm mx-auto">
          Start writing press releases (like new University partnerships) to boost your corporate SEO and attract international partners.
        </p>
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-md rounded-xl shadow-2xl relative z-10 border border-zinc-200 overflow-hidden">
              <div className="p-4 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50">
                <h3 className="font-semibold text-zinc-900">Add Press Release</h3>
                <button onClick={() => setIsModalOpen(false)} className="text-zinc-400 hover:text-zinc-600"><X className="w-4 h-4" /></button>
              </div>
              <div className="p-6">
                <p className="text-sm text-zinc-500 mb-4">You need to run <code>database_update.sql</code> in Supabase to create the `news` table before publishing articles.</p>
